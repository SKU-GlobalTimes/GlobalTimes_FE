...
- PR: https://github.com/SKU-GlobalTimes/GlobalTimes_FE/pull/121
...
- 검증: 로컬 Markdown 상대 링크와 `git diff --check`, Frontend CI `build-and-lint`가 통과했고 AI Reviewer는 Blocking 없음·MERGE_READY로 판정했습니다.
...
- 2차 실제 실행: 무료 HTML preflight를 통과한 영국 RSS 기사도 Backend crawler의 5초 read timeout으로 실패했습니다. Translation 2회, Gemini 0회였고 익명·로그인 질의에는 진입하지 않았습니다.
- 보완: 유료 호출 전에 RSS·기사 HTML을 확인하는 후보 preflight와 External Smoke 전용 crawler timeout 15초를 추가했습니다. 추가 과금 호출 없이 정적 검증 후 Reviewer에게 전달합니다.
- 회귀 검증: 전체 ESLint, Vite production build, PowerShell parser, 변경 E2E lint와 기존 Mock Full Stack E2E `9 passed / External Smoke 1 skipped`가 통과했습니다.
- Reviewer Blocking 반영: External Smoke 종료 시 Backend·Frontend·container 로그의 실제 key 값과 `key=` query를 `[REDACTED]`로 치환하고, compose cleanup을 최대 3회 제한 재시도한 뒤 실패를 명시적으로 보고하도록 보완했습니다.
- Reviewer 재검토 반영: OS 환경변수와 Backend `.env`에 서로 다른 key가 공존해도 양쪽 값을 모두 수집·중복 제거한 뒤 마스킹하도록 key source 경계를 보완했습니다.
...
- Squash commit: https://github.com/SKU-GlobalTimes/GlobalTimes_FE/commit/a0ab366a56b0e3079c798cd702794ba1450bffb7
...
### #108/#109 공개 기사 탐색·비로그인 스크랩 실제 Backend 연동 E2E
...
- Squash commit: https://github.com/SKU-GlobalTimes/GlobalTimes_FE/commit/b45fbc2383b42a983038c47368607c4fe0975fb5
...
- Reviewer Blocking과 조치:
  - popular API가 최근 30일 기사만 반환하므로 고정 발행일 fixture는 시간이 지나면 만료됩니다.
  - 대상 기사를 실행 시각 1시간 전으로 적재하고 재실행 때도 `published_at`을 갱신하도록 보완했습니다.
- 검증 결과:
  - 전체 ESLint와 Vite 7 Production build 통과 (`2,339 modules`)
  - Blocking 반영 후 로컬 Full Stack E2E 4개 통과 (브라우저 `37.5s`, 전체 orchestration `167.2s`)
  - 최신 HEAD GitHub Frontend CI 성공 (`28s`, run `31573621197`)
  - 최신 HEAD GitHub Full Stack E2E 성공 (`1m 24s`, run `31573631635`)
  - AI Reviewer 재검토 `MERGE_READY`, Blocking 없음 확인 후 사용자 승인으로 squash merge
